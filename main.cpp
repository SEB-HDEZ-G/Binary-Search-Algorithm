...
struct Empleado {
    ...
    void printInfo() {
        ...
        cout << "Edad: " << age << endl;
        cout << "Pasatiempos: " << hobbies << endl;
        cout << "Habilidades: " << qualities << endl;
    }
};

void sortVector(vector<Empleado>& empleados) { //usar & para modificar el vector original en lugar de una copia (sin esto no funciona en el main)
    std::sort(empleados.begin(), empleados.end(), [](const Empleado& x, const Empleado& y) {
        return x.name < y.name;
    });
}

int searchByName(vector<Empleado> empleados, string inputName) {
    int low = 0;
...
int main() {
    ...
    sortVector(empleados); //se ordena el vector antes de hacer la búsqueda

    string choice = "S";
    while(choice == "S") { //loop para salir del programa solo si el usuario lo desea
        string inputName;
        cout << "\nIngrese el nombre del empleado: ";
        cin >> inputName;

        int index = searchByName(empleados, inputName);

        //caso base
        if(index != -1) {
            cout << "\nNúmero de empleado: " << index << endl;
            empleados[index].printInfo();
        }
        else {
            cout << "\nNo se encontró al empleado." << endl;
        }
